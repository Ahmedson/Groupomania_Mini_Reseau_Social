<template>
  <div>
    <article :id="post.postImg_id" v-for="post in postsImg" :key="post.postImg_id">
        ...
        <small class="info">
          <h3>
            <img
              ...
              alt=""
            />
            <img v-else src="../assets/profil.png" alt="miniature photo de profil" />
      ...
      <div class="post post--postImg">
        <img :src="post.imageUrl" class="post post--post" /><br />
      </div>
      <div>
        <div v-for="commentObject in arrayCommentObject" :key="commentObject.comment_id">
          <div v-for="comment in commentObject" :key="comment.comment_id">
            ...
            <div
              ...
              v-if="post.postImg_id === comment.postImg_id"
            >
...
export default {
  ...
  computed: {
    ...mapState(["postsImg", "users", "tokenToken", "tokenUserId"]),
  },
  watch: {
    comments(newValue) {
  ...
  methods: {
    ...
    deletePostOrComment(postOrComment) {
      this.$store.dispatch("deletePostOrCommentImgStore", postOrComment);
    },
    editPost(event) {
      this.$store.dispatch("editPostImgStore", event);
    },
    editComment(postOrComment) {
      this.$store.dispatch("editCommentImgStore", postOrComment);
    },
    submitComment(event) {
      event.preventDefault();
      this.$store.dispatch("submitCommentImgStore", event);
    },
  },
  async mounted() {
    await this.$store.dispatch("getToken");
    await this.$store.dispatch("getPostsImg");
    await this.$store.dispatch("getUsers");

    // RÉCUPÈRATION DES COMMENTAIRES DE CHAQUE ARTICLES
    for (let i = 0; i < this.$store.state.postsImg.length; i++) {
      await fetch(
        `http://localhost:3000/commentImg/${this.$store.state.postsImg[i].postImg_id}/`,
        {
          headers: {
